...
import fs from "fs";
import { addFileToIndex, searchFiles } from "../utils/semanticSearch";
...
class FileController {
  ...
  console.log('[AI-RAG] Re-indexing request received.');
  ...
  // --- Semantic Search endpoint (called by frontend AI search button) ---
  semanticSearch = async (
    ...
    try {
      ...
      console.log(`[AI-SEARCH] POST /file-service/semantic-search -> user=${req.user._id}, query="${query}"`);

      const results = await searchFiles(query, 10);

      console.log(`[AI-SEARCH] POST /file-service/semantic-search <- OK in ${Date.now() - startTime}ms, files=${results.files.length}, folders=${results.folders.length}`);
      res.json(results);
    } catch (e: unknown) {
      console.error(`[AI-SEARCH] POST /file-service/semantic-search <- ERROR:`, e);
      next(e);
    }
  };
  ...
  uploadFile = async (
    ...
    try {
      ...
      console.log(`[AI-RAG] Upload complete, triggering indexing for file="${file.filename}", id=${file._id}`);
      addFileToIndex(file).catch((err: any) => {
        console.error(`[AI-RAG] Unhandled error in addFileToIndex for file="${file.filename}":`, err?.message || err);
      });
  ...
  getSuggestedList = async (
    ...
    try {
      ...
      const startTime = Date.now();
      console.log(`[AI-SEARCH] GET /file-service/suggested-list -> user=${userID}, query="${searchQuery}", trashMode=${trashMode}, mediaMode=${mediaMode}`);
      ...
      console.log(`[AI-SEARCH] GET /file-service/suggested-list <- OK in ${Date.now() - startTime}ms, files=${fileList.length}, folders=${folderList.length}`);
