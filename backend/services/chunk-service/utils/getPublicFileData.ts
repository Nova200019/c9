...
import { CloudflareActions } from "../actions/cloudflare-actions";
...
const proccessData = (res: Response, fileID: string, tempToken: string) => {
  ...
  const processFile = async () => {
    try {
      ...
      // Check CDN first
      const cdnUrl = CloudflareActions.getCdnUrl(fileID, file.filename);
      if (cdnUrl) {
        // Redirect client to Cloudflare Edge Cache for immediate download
