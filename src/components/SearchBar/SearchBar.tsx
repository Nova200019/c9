...
const SearchBar = memo(() => {
  ...
  const onSemanticSearch = async () => {
    ...
    try {
      const results = await semanticSearchAPI(searchText);
      // Adapt this if your API returns a different structure
      setSemanticResults({
      ...
      >
        {isSemantic ? (
          <>
            ...
            semanticResults.fileList.length === 0 ? (
              <div className="flex justify-center items-center p-4">
                <span>No AI Results</span>
              </div>
            ) : null}
            {semanticResults.folderList.map((folder: FolderInterface) => (
              <SearchBarItem
                type="folder"
                folder={folder}
            ...
            {semanticResults.fileList.map((file: FileInterface) => (
              <SearchBarItem
                type="file"
                file={file}
                folderClick={folderClick}
